...
GenderData <- ext_survey_df %>% filter(c19_vaccination_status == "Ja") %>%  select(gender) %>%
...
# Processing of external survey data
AgeData <- ext_survey_df %>% filter(c19_vaccination_status == "Ja") %>% select(year_of_birth) %>% mutate(age = 2023-year_of_birth) %>%
...
# Processing of external survey data
HouseholdData <- ext_survey_df %>% filter(c19_vaccination_status == "Ja") %>% select(hsld_size_2019_, hsld_size_03_2020_, hsld_size_summer_2021_, hsld_size_01_2023_, total_hsld_size_persons_under_14, number_of_children_under_18)
...
# Processing of external survey data
Children <- ext_survey_df %>% filter(c19_vaccination_status == "Ja") %>% select(total_hsld_size_persons_under_14) %>%
...
# Processing of external survey data
educationLevel <- ext_survey_df %>% filter(c19_vaccination_status == "Ja") %>% select(highest_educational_qualification) %>%
...
# Processing of external survey data
currentOccupation <- ext_survey_df %>% filter(c19_vaccination_status == "Ja") %>% select(current_occupation) %>%
...
ggsave("./plots/DemographicComparisonVacc.pdf", dpi = 500, w = 24, h = 30)
ggsave("./plots/DemographicComparisonVacc.png", dpi = 500, w = 24, h = 30)
