...
unvacc <- cleaned_data %>% filter(c19_vaccination_status == "Nein") %>%  mutate(
...
vacc <- cleaned_data %>% filter(c19_vaccination_status == "Ja") %>% mutate(
  num_c19_infs_eng = case_when(
    num_c19_infs == "Nie" ~ "0",
    num_c19_infs == "Einmal" ~ "1",
    num_c19_infs == "Zweimal" ~ "2+",
...
infection_categories <- c("0", "1", "2+", "I Don't Want To Answer")
