...
GenderData <- ext_survey_df %>% filter(c19_vaccination_status == "Nein") %>%  select(gender) %>%
...
# Processing of external survey data
AgeData <- ext_survey_df %>% filter(c19_vaccination_status == "Nein") %>% select(year_of_birth) %>% mutate(age = 2023-year_of_birth) %>%
...
# Processing of external survey data
HouseholdData <- ext_survey_df %>% filter(c19_vaccination_status == "Nein") %>% select(hsld_size_2019_, hsld_size_03_2020_, hsld_size_summer_2021_, hsld_size_01_2023_, total_hsld_size_persons_under_14, number_of_children_under_18)
...
# Processing of external survey data
Children <- ext_survey_df %>% filter(c19_vaccination_status == "Nein") %>% select(total_hsld_size_persons_under_14) %>%
...
ggplot(aes(total_hsld_size_persons_under_14, percent)) +
  ...
  geom_errorbar(aes(x=total_hsld_size_persons_under_14, ymin=lci, ymax=uci, colour = factor(Source, levels = c("External Survey", "Federal Statistical Office, Federal Employment Agency", "MuSPAD"))), position = position_dodge(0.8), width = 0.3, alpha=0.9, size=1.3) +
  theme_minimal() +
  theme(plot.margin=unit(c(1,1,1,1), 'cm')) +
...
# Processing of external survey data
educationLevel <- ext_survey_df %>% filter(c19_vaccination_status == "Nein") %>% select(highest_educational_qualification) %>%
...
# Processing of external survey data
currentOccupation <- ext_survey_df %>% filter(c19_vaccination_status == "Nein") %>% select(current_occupation) %>%
...
ggsave("./plots/DemographicComparisonNoVacc.pdf", dpi = 500, w = 24, h = 30)
ggsave("./plots/DemographicComparisonNoVacc.png", dpi = 500, w = 24, h = 30)
